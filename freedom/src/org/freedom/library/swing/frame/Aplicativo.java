		...
		if (!telaPrincipal.temTela(name)) {
			try {
				...
				Funcoes.mensagemErro(framePrinc,
						"erreur null pointeur lors de l'ouverture de l'écran!\n" + "Classe:"
								+ classe + "\n" + "Méthode:" + metodo + "\n"
	...
	public void atualizaMenus() {
		...
		try {
			...
			Funcoes.mensagemErro(null,
					"erreur de mise à jour les Menus !\n" + e.getMessage());
		}

		Funcoes.mensagemInforma(null, "Menus mis à jour avec succès!");
	...
	public DbConnection conexaoIB(String strDriverP, String strBancoP) {
		try {
			...
			if (e.getErrorCode() == 335544472)
				return null;
			Funcoes.mensagemErro(null,
					"[internal]:Impossible de se connecter à la base de données.\n"
							+ e.getMessage());
			return null;
		}
		return conIB;
