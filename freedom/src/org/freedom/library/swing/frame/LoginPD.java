...
public class LoginPD extends Login implements ActionListener, FocusListener {
	...
	public DbConnection getConection() throws Exception {
		...
		// Compara a versăo do banco com a versăo do aplicativo;
		...
		if (( versaosis.compareTo(versaobd) == 0 ) || ( ( "N".equals(Aplicativo.getParameter("validaversao").trim()) ) )) {
			...
			throw new Exception("La version de la base de données (" + versaobd + ") est différente de la version du fichier exécutable (" + versaosis + ")\n"
					+ "Exécuter les procédures de mise à niveau du système, ou contacter le support technique.");
		}
		return conRet;
	...
	protected boolean execConexao(String sUsu, String sSenha) {
		...
		try {
			...
			Funcoes.mensagemErro(this, "Pilote n'a pas été trouvé:\n" + strDriver + "\n" + e.getMessage());
			e.printStackTrace();
			return false;
		}
		...
		try {
			...
			if (e.getErrorCode() == 335544472 || e.getErrorCode() == 335544345)
				Funcoes.mensagemErro(this, "User name or password invalid ! ! !");
			else
				Funcoes.mensagemErro(this, "Impossible d'établir la connexion à la base de données.\n" + e.getMessage());
			e.printStackTrace();
			return false;
		}
		...
		if ("S".equalsIgnoreCase(nfe)) {
			...
			try {
				...
				if (e.getErrorCode() == 335544472 || e.getErrorCode() == 335544345)
					...
					Funcoes.mensagemErro(this, "Impossible d'établir la connexion à la base de données nfe.\n" + e.getMessage());
				e.printStackTrace();
				return false;
			}
		}
	...
	protected boolean montaCombo(String sUsu) {
		...
		try {
			...
			Funcoes.mensagemErro(this, "Erreur de chargement de données de l'entreprise\n" + err);
			err.printStackTrace();
		}
		finally {
			sql = null;
		...
		try {
			...
			for (int i = 0; tabelas.size() > i; i++) {
			    ...
			    try {
			    	...
			    	System.out.println("Impossible d'exécuter le jugement: "+update_table);
			    	con.rollback();
			...
			System.out.println("FINALIZOU ATUALIZAÇƠES!!!");
	...
	protected boolean adicConFilial(DbConnection conX) {
		...
		try {
			...
			// rs = ps.executeQuery();
			// if ( rs.next() ) {
			// System.out.println("1-Conexăo: "+rs.getInt( "CURRENT_CONNECTION"
			// ));
			// }

		}
		catch (SQLException err) {
			Funcoes.mensagemErro(this, "Erreur d'écriture la filiale courante dans la banque!\n" + err.getMessage());
			err.printStackTrace();
		}
		finally {
			rs = null;
